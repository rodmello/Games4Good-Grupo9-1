...
function enableTrackingControls(obj, speed) {
  obj.speed = speed ? speed : 0;
  setXYSpeed(obj);
  obj.distance = 0;
  obj.xTarget = obj.x;
  obj.yTarget = obj.y;
  ...
  obj.setTarget = function (x, y, rotate) {
    if (rotate === undefined) {
      rotate = true;
    }
    // Triangle Geometry
    ...
    obj.xSpeed = obj.speed * cosine;
    obj.ySpeed = obj.speed * sine;
    ...
    let tan = opp / adj;
    let arctan = Math.atan(tan); // result in radians
    if (rotate) {
      obj.angle = arctan * 180 / Math.PI;
      obj.flipY = false;
    }
    else {
      obj.flipX = obj.flip;
...
function enableVectorControls(obj, speed, turnSpeed) {
